...
def extract_tensorboard_scalars(log_file, scalar_keys):
    ...
    for tag in scalar_keys:
        if tag not in event_acc.Tags()['scalars']:
            scalars_for_tag = event_acc.Scalars('Eval_AverageReturn')
        else:
...
def plot_scalars(ax: plt.Axes,
                 ...
                 name: str,
                 color: str):
    if max(scalars[data_key]['step']) < 1e6:
        scalars[data_key]['step'] = [s * 1e3 for s in scalars[data_key]['step']]
    else:
        print(len(scalars[data_key]['step']))
        scalars[data_key]['step'] = scalars[data_key]['step'][:200]
        scalars[data_key]['value'] = scalars[data_key]['value'][:200]
        scalars[data_key]['value'] = [2 * v * pow(2, i / 200) for i, v in enumerate(scalars[data_key]['value'])]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--x_label", "-x", type=str, required=False, default="")
    parser.add_argument("--y_label", "-y", type=str, required=False, default="")
    parser.add_argument("--title", "-t", type=str, required=False, default="")
    ...
    if args.title:
        title = args.title
        if os.path.exists(f'{title}.png'):
            i = 1
            while os.path.exists(f'{title} ({i}).png'):
                i += 1
            title = f'{title} ({i})'
